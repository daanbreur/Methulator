    ...
    if ($stmt = $connection->prepare('SELECT methTransactions.id FROM methTransactions')) {
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                $index = $transaction->transactionDate->format("Y-m-d H:i:s");
                if (!isset($data[0][$index])) {
                    $data['OmzetVoorWitwas'][$index] = $transaction->calculateOmzet();
                    $data['OmzetNaWitwas'][$index] = $transaction->calculateOmzetNaWitwas();
                    $data['MoneyForSlave'][$index] = $transaction->calculateMoneyForSlave();
                    $data['Profit'][$index] = $transaction->calculateProfit();
                } else {
                    $data['OmzetVoorWitwas'][$index] += $transaction->calculateOmzet();
                    $data['OmzetNaWitwas'][$index] += $transaction->calculateOmzetNaWitwas();
                    $data['MoneyForSlave'][$index] += $transaction->calculateMoneyForSlave();
